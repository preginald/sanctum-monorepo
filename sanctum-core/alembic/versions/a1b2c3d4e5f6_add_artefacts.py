...
def upgrade() -> None:
    # Create enums (raw SQL for reliable IF NOT EXISTS)
    op.execute("DO $$ BEGIN CREATE TYPE artefact_type_enum AS ENUM ('file', 'url', 'code_path', 'document', 'credential_ref'); EXCEPTION WHEN duplicate_object THEN NULL; END $$")
    op.execute("DO $$ BEGIN CREATE TYPE artefact_link_entity_type AS ENUM ('ticket', 'account', 'article'); EXCEPTION WHEN duplicate_object THEN NULL; END $$")

    # Create artefacts table
    conn = op.get_bind()
    if not conn.dialect.has_table(conn, 'artefacts'):
        op.create_table(
            'artefacts',
            sa.Column('id', UUID(as_uuid=True), server_default=sa.text("gen_random_uuid()"), primary_key=True),
            sa.Column('name', sa.String(), nullable=False),
            sa.Column('artefact_type', sa.Enum('file', 'url', 'code_path', 'document', 'credential_ref', name='artefact_type_enum', create_type=False), nullable=False),
    ...
    if not conn.dialect.has_table(conn, 'artefact_links'):
        op.create_table(
            'artefact_links',
            sa.Column('id', UUID(as_uuid=True), server_default=sa.text("gen_random_uuid()"), primary_key=True),
            sa.Column('artefact_id', UUID(as_uuid=True), sa.ForeignKey('artefacts.id'), nullable=False),
            sa.Column('linked_entity_type', sa.Enum('ticket', 'account', 'article', name='artefact_link_entity_type', create_type=False), nullable=False),
    ...
    op.execute("CREATE INDEX IF NOT EXISTS ix_trgm_artefact_name ON artefacts USING gin (name gin_trgm_ops)")
