...
def extract_context(t):
    ...
    if ctx:
        ...
        if len(ctx) > 100: ctx = ctx[:97] + "..."
        if not ctx.endswith('.'): ctx += "."
        return ctx
    return ""
    ...
    for proj in projects:
        ...
        for idx, m in enumerate(milestones):
            ...
            if needs_update:
                ...
                if apply_mode:
                    api_put(f"/milestones/{mid}", {"sequence": new_seq, "description": new_desc})
                    total_updated += 1
