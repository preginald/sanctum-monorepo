    ...
    <div className={embedded ? "" : "bg-slate-900 border border-slate-700 rounded-xl p-6"}>
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex items-center gap-3">
            ...
            <div className="flex bg-slate-800 rounded p-0.5 border border-slate-700 mr-2">
                <button onClick={() => toggleView('compact')} className={`p-1 rounded ${viewMode === 'compact' ? 'bg-slate-600 text-white' : 'text-slate-500 hover:text-slate-300'}`}><List size={14} /></button>
                <button onClick={() => toggleView('expanded')} className={`p-1 rounded ${viewMode === 'expanded' ? 'bg-slate-600 text-white' : 'text-slate-500 hover:text-slate-300'}`}><Rows size={14} /></button>
            </div>
      ...
      <div className="space-y-2">
        {visibleTickets.length > 0 ? visibleTickets.map(t => (
            ...
            {viewMode === 'expanded' && (
                <div className="p-3 pr-10">
                    <div className="flex justify-between items-start mb-2">
                        ...
                        <div className={`px-2 py-0.5 rounded text-[10px] uppercase font-bold ${t.status === 'resolved' ? 'bg-green-900 text-green-400' : 'bg-blue-900 text-blue-400'}`}>{t.status}</div>
                    </div>
          ...
          <p className="text-sm opacity-30 italic p-2">No active tasks.</p>
        )}
      </div>
    </div>
  );
