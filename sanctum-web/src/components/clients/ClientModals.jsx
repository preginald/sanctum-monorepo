          ...
          <form onSubmit={onSubmit} className="space-y-4">
              ...
              <div className="flex items-center gap-3 p-3 bg-slate-800/50 rounded border border-slate-700">
                  ...
                  <label htmlFor="portal_access" className="cursor-pointer">
                      <span className="block text-sm font-bold text-white">{forms.contact.id ? 'Portal Access' : 'Enable Portal Access'}</span>
                      <span className="block text-xs text-slate-500">
                          {forms.contact.id
                              ? 'Toggle portal access for this contact. Enabling will provision a user account if needed.'
                              : 'Automatically creates a user account and sends an invite email.'}
                      </span>
                  </label>
              </div>
