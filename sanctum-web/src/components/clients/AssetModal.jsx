...
export default function AssetModal({ isOpen, onClose, onSubmit, loading, form, setForm }) {
  ...
  // Logic: Is this a "Digital" asset?
  const isDigital = ['domain', 'hosting_web', 'hosting_email', 'license', 'software', 'saas'].some(t => form.asset_type?.includes(t));
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={form.id ? "Edit Asset" : "Deploy Asset"}>
        <form onSubmit={onSubmit} className="flex flex-col h-full">
            ...
            {/* SCROLLABLE BODY: Constrain height to ensure modal fits on laptops */}
            <div className="space-y-4 overflow-y-auto max-h-[60vh] pr-2 -mr-2 custom-scrollbar">
                
                {/* ROW 1: CONTEXT (Type & Status) - Moved to Top */}
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <label className="text-xs opacity-50 block mb-1">Type</label>
                        <select 
                            autoFocus
                            className="w-full p-2 bg-slate-800 border border-slate-600 rounded text-white capitalize focus:border-sanctum-blue outline-none" 
