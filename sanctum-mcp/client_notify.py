"""HTTP client for Sanctum Notify API — used by tools/notify.py.

Structural mirror of client.py: same shared httpx.AsyncClient with connection
pooling, same retry/backoff on transient errors (502/503/504,
ConnectError/ReadError/WriteError/PoolTimeout), same 422-passthrough on
POST/PUT, same redaction logic in _check_upstream.

Reuses MCP_MAX_CONNECTIONS / MCP_MAX_KEEPALIVE from client.py (do NOT
introduce a second pool-config pair).

Authenticates to the Notify API using the NOTIFY_API_TOKEN env var — a raw
bearer token that Notify hashes server-side (SHA-256) to look up an ApiKey
row. The token's tenant_id scopes all reads and writes; MCP tools do NOT
...
API_BASE = os.getenv("NOTIFY_API_BASE", "https://notify.digitalsanctum.com.au/api")
API_TOKEN = os.getenv("NOTIFY_API_TOKEN", "")
CURRENT_API_TOKEN: ContextVar[str] = ContextVar("current_notify_api_token", default=API_TOKEN)
...
# Reuse the same pool-config envs as client.py — see .env.example note.
...
async def _get_client() -> httpx.AsyncClient:
    ...
    if _client is None or _client.is_closed:
        log.info("Notify HTTP pool: max_connections=%d, max_keepalive=%d", _MAX_CONN, _MAX_KEEPALIVE)
...
def _check_upstream(r: httpx.Response, method: str, path: str) -> None:
    ...
    if r.status_code in (401, 403):
        ...
        body = r.text[:200]
        log.error(
            "NOTIFY UPSTREAM %d | %s %s | token=%s | body=%s",
